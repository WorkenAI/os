...
export function getDomainViewSpecId(
	domain: DomainDefinition,
	viewId: string | null | undefined,
) {
...
export function isLocalOnlyView(
	domain: DomainDefinition,
	viewId: string | null | undefined,
) {
...
export function getNavigationItemByViewId(
	...
	if (!viewId) return null;
	return (
		domain.surfaces.navigation.find((item) => item.viewId === viewId) ?? null
	);
}
...
export function getNavigationItemBySurfaceId(
	...
	return (
		domain.surfaces.navigation.find((item) => item.id === surfaceId) ?? null
	);
}
...
export function getViewLabel(
	...
	return (
		getNavigationItemByViewId(domain, viewId)?.label ??
		getDomainView(domain, viewId)?.title ??
		null
	);
}
...
export function buildLocalSurfaceAssistantText(
	...
	return [
		interpolateTemplate(
			shell?.localAssistant?.introTemplate ??
				"Working inside the local surface {viewLabel}.",
...
export function getEntityForView(
	domain: DomainDefinition,
	viewId: string | null | undefined,
) {
...
export function listDomainViews(
	domain: DomainDefinition,
): DomainViewDefinition[] {
	return Object.values(domain.views);
}

export function getDomainActionCards(
	domain: DomainDefinition,
): readonly ActionCardAuthoring[] {
	return domain.actionCards ?? [];
}
