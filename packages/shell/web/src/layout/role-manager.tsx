...
import type { DomainPermissions, RoleDefinition } from '@worken/shell-web/permissions/types'
import {
  getDomainEntities,
  getDomainMeta,
  getDomainSidebarItems,
  getDomainVerbLabels,
} from '@worken/shell-web/runtime/domain/catalog'
...
import { createRoleManagerMachine } from '@worken/shell-web/machines/role-manager-machine'
...
export function RoleManagerProvider({ children }: { children: ReactNode }) {
  ...
  const roleMachineRef = useRef<ReturnType<typeof createRoleManagerMachine> | null>(null)
  if (!roleMachineRef.current) {
    roleMachineRef.current = createRoleManagerMachine(
      currentRole.id,
      defaultDomainId as (typeof DOMAIN_IDS)[number],
    )
  }
  const [snapshot, send] = useMachine(roleMachineRef.current)
  const editingRoleId = snapshot.context.editingRoleId
  const selectedDomainId = snapshot.context.selectedDomainId

  useEffect(() => {
    if (roles.some((role) => role.id === editingRoleId)) return
    send({ type: 'editing.set', roleId: defaultEditingRoleId })
  }, [defaultEditingRoleId, editingRoleId, roles, send])

  useEffect(() => {
    if (DOMAIN_IDS.includes(selectedDomainId as (typeof DOMAIN_IDS)[number])) return
    send({ type: 'domain.set', domainId: defaultDomainId as (typeof DOMAIN_IDS)[number] })
  }, [defaultDomainId, selectedDomainId, send])
  ...
  const api = useMemo<RoleManagerApi | null>(() => {
    ...
    return {
      ...
      selectedDomainId,
      createRole,
      setEditingRoleId: (roleId) => send({ type: 'editing.set', roleId }),
      setSelectedDomainId: (domainId) =>
        send({ type: 'domain.set', domainId: domainId as (typeof DOMAIN_IDS)[number] }),
      resetToDefaults,
