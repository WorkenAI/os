...
import { createAgentManagerMachine } from '@worken/shell-web/machines/agent-manager-machine'
...
export function AgentManagerProvider({ children }: { children: ReactNode }) {
  const agentMachine = useMemo(
    () => createAgentManagerMachine(DEMO_AGENTS[0]?.id ?? null),
    [],
  )
  const [snapshot, send] = useMachine(agentMachine)
  const selectedAgentId = snapshot.context.selectedAgentId
  const inspectorTab = snapshot.context.inspectorTab
  ...
  const api = useMemo<AgentManagerApi>(
    () => ({
      ...
      inspectorTab,
      audit: DEMO_AUDIT,
      selectAgent: (id) => send({ type: 'agent.select', id }),
      setInspectorTab: (tab) => send({ type: 'inspector.tab', tab }),
    }),
    [inspectorTab, selectedAgentId, send],
  )
