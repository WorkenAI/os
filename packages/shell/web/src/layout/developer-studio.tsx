...
import { MessageRenderer } from '@worken/shell-web/chat/message-renderer'
import { buildExecutionPolicyUrl, buildExecutionRunsUrl } from '@worken/shell-web/runtime/session/transport'
...
import {
  developerStudioInspectorMachine,
  developerStudioMachine,
  type WorkspaceId,
} from '@worken/shell-web/machines/developer-studio-machine'
...
export function DeveloperStudioProvider({ children }: { children: ReactNode }) {
  const [snapshot, send] = useMachine(developerStudioMachine)
  const activeWorkspaceId = snapshot.context.activeWorkspaceId

  const api = useMemo<DeveloperStudioApi>(
    () => ({
      activeWorkspaceId,
      setActiveWorkspaceId: (workspaceId) => send({ type: 'workspace.set', id: workspaceId }),
        ...
        <div className="mx-auto max-w-4xl">
          <div className="relative flex items-end rounded-3xl bg-zinc-900/55 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] transition-colors focus-within:bg-zinc-900/70">
            <textarea
              value={input}
              onChange={(event) =>
                sendInput({ type: 'input.change', value: event.target.value })
              }
...
export function DeveloperStudioInspectorPanel() {
  ...
  const [inspectorSnapshot, sendInspector] = useMachine(developerStudioInspectorMachine)
  const policy = inspectorSnapshot.context.policy
  const activeCase = inspectorSnapshot.context.activeCase
  const isSaving = inspectorSnapshot.context.isSaving
  ...
  const refreshRuntimeState = useCallback(async () => {
    ...
    if (policyResponse.ok) {
      ...
      sendInspector({ type: 'policy.set', policy: policyPayload.policy })
    }

    if (caseResponse.ok) {
      const casePayload = (await caseResponse.json()) as { case: StoredExecutionCase | null }
      sendInspector({ type: 'case.set', case: casePayload.case })
    }
  }, [primaryRuntimeDomainId, primaryRuntimeViewId, resolvedSignalType, sendInspector])
  ...
  const updatePolicy = useCallback(
    async (updater: (current: WorkenAiDelegationPolicy) => WorkenAiDelegationPolicy) => {
      ...
      sendInspector({ type: 'policy.set', policy: nextPolicy })
      sendInspector({ type: 'saving.set', value: true })
      ...
      try {
        ...
        sendInspector({ type: 'saving.set', value: false })
      }
    },
    [policy, resolvedSignalType, sendInspector],
  )
  ...
  const emitRestartSignal = useCallback(async () => {
    ...
    primaryRuntimeDomainId,
    primaryRuntimeViewId,
    refreshRuntimeState,
    resolvedSignalType,
    sendInspector,
  ])
