...
import { getDomainViewExecution, loadDomain } from '@worken/demo-data/shell-contract'
import {
  applyExecutionVerb,
  ensureExecutionRun,
...
import { enrichShellSpec } from '@/shell/server/semantic'
      ...
      return new Response(
        createShellStream(async (writer) => {
          ...
          if (specBlock?.type === 'spec') {
            ...
            const enriched = enrichShellSpec(specBlock.spec as Spec, {
              permissions,
              domainId: domain.id,
              domainTitle: domain.title,
              shellSession: runtimeSession.shellSession,
              object: caseRecord ? { ...caseRecord.state } : undefined,
            })
            await writer.spec((enriched ?? specBlock.spec) as never, {
