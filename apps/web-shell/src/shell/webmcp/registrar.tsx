...
import { shellDomainHref } from "@/shell/shell-paths";
import {
...
export function WorkenOsWebMcpRegistrar() {
	...
	const generatedTools = useMemo(
		() => generateWorkenOsTools(snapshot),
		[snapshot],
	);
	...
	const runtimeRef = useRef<ToolRuntimeState>({
		...
		openDomain: (domainId) => router.push(shellDomainHref(domainId)),
	...
	useEffect(() => {
		...
		const nextSignatures = new Map(
			generatedTools.map((tool) => [
				tool.name,
				JSON.stringify(normalizeToolSignature(tool)),
			]),
		);
		...
		for (const tool of generatedTools) {
			...
			modelContext.registerTool(
				toRegisteredTool(tool, runtimeRef, snapshotRef),
			);
	...
	switch (tool.action.kind) {
		...
		case "list_current_domain_capabilities":
			return createToolResult(
				"Listed visible capabilities in the current Worken OS domain.",
				{
					roleName: snapshot.currentRoleName,
		...
		case "open_domain": {
			const domainId =
				tool.action.domainId ?? readRequiredString(input, "domainId");
			if (!domainId) {
				return createToolResult(
					"Cannot open a domain because no domain id was provided.",
					{
			...
			return createToolResult(`Opened the ${domainId} domain in Worken OS.`, {
				domainId,
			});
		}
		case "open_view": {
			...
			if (!viewId) {
				return createToolResult(
					"Cannot open a view because no view id was provided.",
					{
						status: "invalid_input",
					},
				);
		...
		case "execute_verb": {
			...
			if (!verbId) {
				return createToolResult(
					"Cannot execute an action because no verb id was provided.",
					{
						status: "invalid_input",
					},
				);
			...
			const entityType =
				tool.action.entityType ?? readOptionalString(input, "entityType");
			...
			if (!confirmed) {
				return createToolResult(
					"The requested Worken OS action was cancelled by the user.",
					{
						status: "cancelled",
						verbId,
						entityType,
		...
		case "open_inspector": {
			...
			if (!entityType || !id) {
				return createToolResult(
					"Cannot open the inspector because entityType or id is missing.",
					{
						status: "invalid_input",
					},
				);
			...
			return createToolResult(
				`Opened ${entityType} ${id} in the Worken OS inspector.`,
				{
					entityType,
					id,
				},
		...
		case "send_message": {
			...
			if (!text) {
				return createToolResult(
					"Cannot send a message because no text was provided.",
					{
						status: "invalid_input",
					},
				);
