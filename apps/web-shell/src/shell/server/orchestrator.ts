...
import { enrichShellSpec, type ShellSemanticEnrichmentInput } from '@/shell/server/semantic'
import {
  canAccessDomain,
...
export type ShellStreamRequest = {
  ...
  /** WorkSession from `resolveShellSession` — required for enriched specs aligned with README. */
...
function shellSemanticInput(
  domain: { id: string; title: string },
  permissions: PermissionSnapshot | null,
  shellSession: WorkenOsSession | null,
): ShellSemanticEnrichmentInput {
  return {
    permissions,
    domainId: domain.id,
...
function resolveSpec(
  ...
  return {
    viewId,
    specId,
    spec: enrichShellSpec(normalizedSpec, shellSemanticInput(domain, permissions, shellSession)),
  }
}
...
async function buildBootstrapResponse(
  ...
  const preferredSpec = resolveSpec(domain, preferredViewId, permissions, shellSession)
...
async function buildViewResponse(
  ...
  const resolved = resolveSpec(domain, viewId, permissions, shellSession)
...
async function buildVerbResponse(
  ...
  const resolved = resolveSpec(
    domain,
    getAllowedViewId(domain, permissions, input.activeView),
    permissions,
    shellSession,
  )
...
export async function buildShellStreamResponse(request: ShellStreamRequest): Promise<StreamResponse> {
  const domain = await loadDomain(request.domainId)
  const { shellSession } = request

  if (request.mode === 'bootstrap') {
    return buildBootstrapResponse(domain, request.permissions, shellSession)
  }
  ...
  switch (request.mode) {
    case 'view':
      return buildViewResponse(domain, request.permissions, shellSession, request.specId)
    case 'verb':
      return buildVerbResponse(domain, request.permissions, shellSession, {
        verb: request.verb,
    ...
    case 'user': {
      ...
      return buildUserResponse(domain, request.permissions, shellSession, prompt, request.activeView)
    }
    default:
      return {
