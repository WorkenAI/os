...
export function CrmManagerProvider({ children }: { children: ReactNode }) {
  const [selectedBindingId, setSelectedBindingId] = useState<string | null>(
    DEMO_BINDINGS[0]?.id ?? null,
  )
  const [inspectorSection, setInspectorSection] = useState<InspectorSection>('fields')
  ...
  const api = useMemo<CrmManagerApi>(
    () => ({
      ...
      selectedBindingId,
      inspectorSection,
      selectBinding: setSelectedBindingId,
      setInspectorSection,
    }),
    [selectedBindingId, inspectorSection],
  )
