...
import {
	useCurrentInspectorEntity,
	useShellSession,
} from "@/shell/session/context";
...
export function InfoPanel() {
				...
				...Object.values(domain.verbs)
					.filter(
						(verb) =>
							...
							!domain.surfaces.actions.some(
								(action) => action.verbId === verb.id,
							),
					)
					.map((verb) => ({
						id: `verb:${verb.id}`,
	...
	const visibleActions = domain
		? actions.filter(
				(action) =>
					!action.entityType ||
					canExecuteEntityAction(domain.id, action.entityType, action.verb),
			)
		: [];
					...
					<div className="p-4">
						...
						<span
							className={cn(
								...
								statusColors[resolvedEntity.status] ??
						...
						<div className="space-y-3">
							<InfoField
								icon={Mail}
								label="Email"
								value={resolvedEntity.email}
							/>
							<InfoField
								icon={MapPin}
								label="Location"
								value={resolvedEntity.location}
							/>
							<InfoField
								icon={Briefcase}
								label="Source"
								value={resolvedEntity.source}
							/>
							<InfoField
								icon={Calendar}
								label="Applied"
								value={resolvedEntity.appliedDate}
							/>
						</div>
