...
const WAYPOINTS: Record<
  string,
  { spawn: Pt; descend: Pt; center: Pt; midway: Pt; department: Pt }
> = {
  hr: {
  ...
  marketing: {
    spawn: { x: 0, y: -290, s: 0.86 },
    descend: { x: 0, y: -170, s: 0.94 },
    center: { x: -86, y: 56, s: 1 },
    midway: { x: -150, y: 116, s: 1 },
...
const systemApps = [
  {
    ...
    caption: 'Roles + Access',
    position: 'left-[13%] top-[33%]',
    accent: DOMAIN_PRIMARY_COLORS.admin,
  ...
  },
  {
    ...
    accent: DOMAIN_PRIMARY_COLORS.developer,
...
const businessDomains = [
  {
    ...
    accent: DOMAIN_PRIMARY_COLORS.hr,
  ...
  },
  {
    ...
    accent: DOMAIN_PRIMARY_COLORS.sales,
  ...
  },
  {
    ...
    accent: DOMAIN_PRIMARY_COLORS.marketing,
  ...
  },
  {
    ...
    accent: DOMAIN_PRIMARY_COLORS.finance,
    background: 'linear-gradient(135deg, rgba(245, 158, 11, 0.18), rgba(245, 158, 11, 0.05))',
  },
] as const

type SceneDomainId = (typeof systemApps)[number]['id'] | (typeof businessDomains)[number]['id']

function RobotVisual({ accent, isLight }: { accent: string; isLight: boolean }) {
  return (
    <div
    ...
    >
      <span
        ...
        style={{
          ...
          background: isLight ? 'rgba(250, 246, 241, 0.92)' : 'rgba(0, 0, 0, 0.7)',
          color: isLight ? 'rgba(59, 35, 19, 0.75)' : 'rgba(255, 255, 255, 0.75)',
        }}
      >
        AI
      </span>
      <div
        ...
        style={{
          borderColor: isLight ? 'rgba(120, 80, 40, 0.12)' : 'rgba(255, 255, 255, 0.1)',
          background: isLight ? 'rgba(120, 80, 40, 0.05)' : 'rgba(255, 255, 255, 0.05)',
    ...
    >
      <div
        ...
        style={{
          ...
          background: isLight ? 'rgba(250,246,241,0.18)' : 'rgba(255,255,255,0.03)',
...
function DomainDeliveryLayer({
  ...
  isLight: boolean
  onPhaseEnd: (domainId: LandingSceneDomainId, phase: LandingDeliveryPhase) => void
  ...
  const wp = WAYPOINTS[delivery.domainId]!
  const showPacket = delivery.phase === 'fetching' || delivery.phase === 'delivering'
    ...
    if (delivery.phase === 'fetching') {
      if (robot) {
        const a = robot.animate(
          [toTransform(wp.department), toTransform(robotMidway), toTransform(robotCenter)],
      ...
      if (packet) {
        anims.push(
          packet.animate([toTransform(wp.spawn), toTransform(wp.descend), toTransform(wp.center)], {
            duration: SPAWN_MS,
            easing: 'ease-out',
            fill: 'forwards',
          }),
  ...
  return (
    <>
      ...
      {showPacket && (
        <div ref={packetRef} className="absolute left-[50%] top-[50%] z-20 h-0 w-0">
          <PacketVisual
    ...
    >
      ...
      >
        <div
          ...
          style={{
            borderColor: isLight ? 'rgba(120, 80, 40, 0.12)' : 'rgba(255, 255, 255, 0.08)',
            background: isLight ? 'rgba(120, 80, 40, 0.04)' : 'rgba(255, 255, 255, 0.025)',
          }}
        />
        <div
        ...
        <div
          ...
          style={{ ...roleLayerStyle, pointerEvents: isRoleLayerActive ? 'auto' : 'none' }}
        >
          {systemApps.map((app) => (
            <button
              type="button"
            ...
            >
              <div className="mb-2 flex items-center justify-between">
                ...
                <span
                  ...
                  style={{ background: app.accent, boxShadow: `0 0 14px ${app.accent}` }}
                />
              </div>
        ...
        <div
          className="absolute inset-0"
          style={{ ...businessLayerStyle, pointerEvents: isRoleLayerActive ? 'none' : 'auto' }}
        >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  boxShadow:
                    hoveredDomainId === domain.id || activeDomainId === domain.id
              ...
              >
                ...
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span
                    className="h-2.5 w-2.5 rounded-full"
                    style={{ background: domain.accent, boxShadow: `0 0 14px ${domain.accent}` }}
                  />
                </div>
